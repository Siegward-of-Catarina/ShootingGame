...
namespace myge
{
	std::vector<entt::entity> EntityFactory::createEntities(sdl_engine::GameContext& context_, json& data_)
	{
		...
		if (data_.contains("Hud")) {
			for (auto& hud_data : data_.at("Hud")) { entities_data.emplace_back(hud_data); }
		}
		...
		for (i32 i{ 0 }; auto & entity_data : entities_data)
		{
			...
			// [PlayerTag]
			if (entity_data.contains("PlayerTag")) { registry.emplace<PlayerTag>(entities[i]); }
			// [EnemyTag]
			if (entity_data.contains("EnemyTag")) { registry.emplace<EnemyTag>(entities[i]); }
			// [BackgroundTag]
			if (entity_data.contains("BackgroundTag")) { registry.emplace<BackgroundTag>(entities[i]); }
			i++;
		}
