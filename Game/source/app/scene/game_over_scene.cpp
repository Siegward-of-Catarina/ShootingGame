...
myge::GameOverScene::~GameOverScene()
{
   ...
   // 本シーン所属のエンティティを破棄
   auto& reg { registry() };
   for ( auto entt : reg.view<AffilGameOverScene>() )
   {
...
void myge::GameOverScene::update( const sdl_engine::FrameData& ) {}
...
void myge::GameOverScene::createEntities()
{
   ...
   if ( auto data { sdl_engine::tryGetJson( scene_data, "Entities" ) }; data )
   {
      ...
      factory.createEntities( *data, typeid( AffilGameOverScene ) );
   }
}
