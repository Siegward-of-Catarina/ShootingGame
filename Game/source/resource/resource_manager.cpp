...
namespace
{
	...
	u32 u32Hash(std::string_view key) {
		return static_cast<u32>(std::hash<std::string_view>()(key.data()));
	}
}    // namespace
...
namespace myge
{
	...
	{
		if (entt::resource<SDL_Texture> res = _sprite_cache[u32Hash(key)]; res)
		{
			return res;
		}
		return {};
	...
	entt::resource<SpriteAnimResource> ResourceManager::getSpriteAnim(std::string_view key)
	{
		...
		if (entt::resource<SpriteAnimResource> res = _sprite_anim_cache[u32Hash(key)]; res)
		{
			return res;
		}
		return {};
