...
自作の軽量2D エンジン上で、ECS 設計（Entity-Component-System）を用いた拡張しやすい構造を採用し、ゲームプレイとエンジン設計の両面を検証しています。
...
## 🎯　開発の目的
- **ECSアーキテクチャの実践的学習**
  - 専門学校でのOOP開発で経験した継承構造の複雑化を解決するため、ECS設計を採用
  - コンポーネントの組み合わせによる柔軟なエンティティ生成を実現
- **データ駆動開発の検証**
  - ゲームロジックとデータを分離し、JSONによる外部管理を実践
  - 敵の出現パターンやシーン構成をコード変更せずに調整可能
- **軽量の2Dエンジン制作に挑戦**
  - SDL3をベースに、必要最低限の機能を持つエンジンを自作
  - エンジン層とアプリケーション層を明確に分離し、再利用性を重視 
---

## 📐 アーキテクチャ

BioArmadaのシステム設計とアーキテクチャの詳細については、[アーキテクチャ図ドキュメント](docs/architecture.md)をご覧ください。
...
- Game/
   ...
   - source/
     ...
     - engine/ … エンジン層（コア/レンダリング/サウンド/マネージャ/一般・エンジン機能システム/ユーティリティ）
...
1) エンジン初期化時に [assets/assets.json](Game/assets/assets.json) を読み込み 
...
- 攻撃/衝突/ダメージ
  ...
  - CollisionSystem: グリッド分割による衝突判定、HitEvent を発火
...
- アセット定義: [Game/assets/assets.json](Game/assets/assets.json)
 ...
 - Sounds: `name` → 音声ファイル
 - FontDatas: テクスチャフォントの1文字矩形配列

- シーン・ウェーブ
 - scene_data: エンティティ群（背景/UI/テキスト/メニュー/プレイヤ/BGMなど）を JSONで定義
 - wave_data: 敵の出現パターン（wanderer/shooter/boss）や演出（SE/点滅）を JSONで定義
